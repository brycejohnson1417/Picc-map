...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const ranges = Array.isArray(req.body?.ranges)
    ? req.body.ranges.filter((value: unknown): value is string => typeof value === 'string' && value.trim().length > 0)
    : [];
  ...
  try {
    const response = await sheetsFetch(
      `spreadsheets/${encodeURIComponent(sheetId)}/values:batchGet`,
      {
        ranges,
        majorDimension: req.body?.majorDimension || 'ROWS',
        valueRenderOption: req.body?.valueRenderOption || 'FORMATTED_VALUE',
        dateTimeRenderOption: req.body?.dateTimeRenderOption || 'SERIAL_NUMBER',
      },
    );
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : 'Failed to fetch Google Sheets batch data',
    });
  }
}
