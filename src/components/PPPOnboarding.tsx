  ...
  const moveNext = (id: string, currentStatus: PPPStatus) => {
    ...
    if (currentIndex < statuses.length - 1) {
      ...
      setDispensaries((prev) =>
        prev.map((dispensary) => (dispensary.id === id ? { ...dispensary, pppStatus: nextStatus } : dispensary))
      );
    }
  };
        ...
        <div>
          ...
          <div className="mt-4 flex gap-4 text-sm">
            <div className="flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-200 rounded text-amber-800">
              ...
              <span>
                <strong>Bottleneck Alert:</strong> {dispensaries.filter((d) => d.pppStatus === 'API Key Needed').length} stores waiting for API Keys.
              </span>
            </div>
            {hasLoadError && (
              <div className="flex items-center gap-2 px-3 py-1 bg-orange-50 border border-orange-200 rounded text-orange-800">
                <AlertTriangle size={14} />
                <span>Live sheet sync unavailable. Showing fallback dataset.</span>
              </div>
            )}
          </div>
        </div>
            ...
            <div key={status} className="w-80 flex flex-col">
              <div className={`flex items-center justify-between p-3 rounded-t-lg border-b-2 font-semibold text-sm ${
                ...
                <span className="bg-white/50 px-2 py-0.5 rounded text-xs">
                  {dispensaries.filter((item) => item.pppStatus === status).length}
                </span>
              </div>
              <div className="flex-1 p-2 bg-slate-50/50 rounded-b-lg border-x border-b border-slate-200 space-y-3 overflow-y-auto max-h-[calc(100vh-250px)] custom-scrollbar">
                {dispensaries
                  .filter((dispensary) => dispensary.pppStatus === status)
                  .map((dispensary) => (
                    <div
                      key={dispensary.id}
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-semibold text-slate-800 line-clamp-1" title={dispensary.name}>
                          {dispensary.name}
                        </h4>
                ...
                {dispensaries.filter((item) => item.pppStatus === status).length === 0 && (
